...
class CartActiveEventTest {

    @Test
    void constructor_shouldHoldCartAndCustomer_GreenPath() {
        // Given
        Cart cart = new CartDefault(UUID.randomUUID());
        Customer customer = new CustomerDefault(UUID.randomUUID(), "John Doe", "john@example.com");

        // When
        CartActiveEvent event = new CartActiveEvent(cart, customer);
    ...
    void constructor_whenNullParameters_shouldAllowNulls_RedPath() {
        // When
        CartActiveEvent event = new CartActiveEvent(null, null);
